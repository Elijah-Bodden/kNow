...
The `when` function automatically attatches a callback, the second argument, to be run every time an event is dispatched with an identifier equal to the first argument. The function returns an `instanceID` object which can be passed into `clearWhen` to destroy the listener.
## clearWhen(object | string | number | undefined)
`clearObject` accepts as a parameter either a string or a number—which will unregister all `when` callbacks registered to that type—an `instanceID` generated by calling `when`—which will remove only the callback attatched by this call—or no value, in which case all registered callbacks will be destroyed simultaneously.
## dispatch(number | string, any)
Calling `dispatch` will immediately resolve all promises which were registered through `next` with the first argument as their ID, and invoke all callbacks attatched to the value by `when`. All such promises will resolve to the value provided as the second argument here, and it will be passed to all callback functions.
## forceReject(number | string, any)
`forceReject` does the exact opposite of `dispatch`, batch-rejecting all promises with corresponding ID's and setting the `reason` values to the second argument.
## next(number | string, number | undefined)
The `next` method returns a promise which may either be resolved by `dispatching` the value provided as the first parameter to this function, or rejected through `clearNext` or `forceReject`. Optionally, a second argument, a timeout, may be specified, after which to automatically reject to function. If this argument is deferred, it will default to the default provided to the constructor, and if this too is undefined, there will be no timeout.
## clearnext(...string | number | undefined)
Accepts a list of dispatch ID's; if none are provided, the function defaults to every ID with a registered `next` promise. Each promise registered to each of these ID's is `forceReject`ed with the reason "flushed".
## in(number)
Creates a promise which will resolve to undefined in the specified number of milliseconds.
## clearIn
