...
class kNow {
	...
	clearWhen(clearParameter) {
		...
		if (clearParameter.id) {
			var loc = Object.keys(this.persistentHandlers).reduce((last, current, index) => {
				var includedIndex = this.persistentHandlers[current].reduce(
					(last, entry, index) => (entry.instanceID === clearParameter.id ? index : last),
					false
				);
	...
	dispatch(signalIdentifier, externalDetail) {
		if (this.persistentHandlers[signalIdentifier]) {
			this.persistentHandlers[signalIdentifier].forEach((member) => member.method(externalDetail));
		}
			...
			this.dispatchWatchers[dispatchIdentifier].push({
				promise: new Promise((resolve, reject) => {
					...
					setTimeout(
						...
						timeout !== undefined && timeout !== null ? timeout : this.defaultNextTimeout
					);
				}),
	...
	async in(interval) {
		try {
			await this.next("&&IN_RESERVED_NEXT&&", interval);
		} catch {}
	}
	async clearIn() {
